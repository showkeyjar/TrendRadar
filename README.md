...
> 🛠️ 本地调试开发请看：[本地开发快速启动](docs/local-dev-quickstart.md)
...
默认是双服务部署（`trendradar` + `trendradar-mcp`），也支持单容器部署（`trendradar-single`）：

| 服务名称 | 用途 | 说明 |
|---------|------|------|
| `trendradar` | 新闻推送服务 | 定时抓取新闻、推送通知（必选） |
| `trendradar-mcp` | AI 分析服务 | MCP 协议支持、AI 对话分析（可选） |
| `trendradar-single` | 单容器模式 | 抓取调度 + Web 控制台 + MCP 同容器运行（简化部署） |
